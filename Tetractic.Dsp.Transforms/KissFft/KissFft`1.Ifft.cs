...
#pragma warning disable IDE1006 // Naming Styles
        ...
        private static void kf_bfly2(Span<kiss_fft_cpx> fOut, int fStride, ReadOnlySpan<kiss_fft_cpx> twiddles, int m)
        {
            ...
            do
            {
                temp1 = fOut[m] * Conjugate(twiddles[tw1]);
                tw1 += fStride;
        ...
        private static void kf_bfly4(Span<kiss_fft_cpx> fOut, int fStride, ReadOnlySpan<kiss_fft_cpx> twiddles, int m)
        {
            ...
            {
                temp1 = fOut[m] * Conjugate(twiddles[tw1]);
                temp2 = fOut[2 * m] * Conjugate(twiddles[2 * tw1]);
                temp3 = fOut[3 * m] * Conjugate(twiddles[3 * tw1]);
                tw1 += fStride;
        ...
        private static void kf_bfly3(Span<kiss_fft_cpx> fOut, int fStride, ReadOnlySpan<kiss_fft_cpx> twiddles, int m)
        {
            ...
            T epi3 = Conjugate(twiddles[fStride * m]).i;
            ...
            {
                temp1 = fOut[m] * Conjugate(twiddles[tw1]);
                temp2 = fOut[2 * m] * Conjugate(twiddles[tw1 * 2]);
                tw1 += fStride;
        ...
        private static void kf_bfly5(Span<kiss_fft_cpx> fOut, int fStride, ReadOnlySpan<kiss_fft_cpx> twiddles, int m)
        {
            ...
            var ya = Conjugate(twiddles[fStride * m]);
            var yb = Conjugate(twiddles[2 * fStride * m]);
            ...
            do
            {
                ...
                temp2 = fOut[2 * m] * Conjugate(twiddles[2 * tw1]);
                temp3 = fOut[3 * m] * Conjugate(twiddles[3 * tw1]);
                temp4 = fOut[4 * m] * Conjugate(twiddles[4 * tw1]);
                tw1 += fStride;
            ...
            for (int u = 0; u < m; ++u)
            {
                ...
                for (int q1 = 0; q1 < p; ++q1)
                {
                    ...
                    for (int q = 1; q < p; ++q)
                    {
                        ...
                        temp += scratch[q] * Conjugate(twiddles[twidx]);
                    }
                    fOut[k] = temp;
...
#pragma warning restore IDE1006 // Naming Styles
    ...
    private static kiss_fft_cpx Conjugate(kiss_fft_cpx a)
    {
